...
// call this function only on server...
export function getAllMoviesOnServer(page = 1, searchTerm = '') {
...
// call this function only on client...
export function getAllMovies(page = 1, searchTerm = '') {
  return fetch(
    `/api/movies?page=${page}&query=${encodeURIComponent(searchTerm)}`
  ).then((res) => res.json());
}
